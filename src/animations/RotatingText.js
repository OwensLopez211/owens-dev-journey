...
const RotatingText = forwardRef(({ texts, transition = { type: "spring", damping: 25, stiffness: 300 }, initial = { y: "100%", opacity: 0 }, animate = { y: 0, opacity: 1 }, exit = { y: "-120%", opacity: 0 }, animatePresenceMode = "wait", animatePresenceInitial = false, rotationInterval = 2000, staggerDuration = 0, staggerFrom = "first", loop = true, auto = true, splitBy = "characters", onNext, mainClassName, splitLevelClassName, elementLevelClassName, ...rest }, ref) => {
    ...
    const splitIntoCharacters = (text) => {
        if (typeof Intl !== "undefined" && "Segmenter" in Intl) {
            try {
                ...
                console.warn("Intl.Segmenter is not supported, falling back to simple split.");
            }
        }
    ...
    return (_jsxs(motion.span, { className: cn("flex flex-wrap whitespace-pre-wrap relative", mainClassName), ...rest, layout: true, transition: transition, children: [_jsx("span", { className: "sr-only", children: texts[currentTextIndex] }), _jsx(AnimatePresence, { mode: animatePresenceMode, initial: animatePresenceInitial, children: _jsx(motion.div, { className: cn(splitBy === "lines"
                        ? "flex flex-col w-full"
                        : "flex flex-wrap whitespace-pre-wrap relative"), layout: true, "aria-hidden": "true", children: elements.map((wordObj, wordIndex, array) => {
